...
function GraphViewInner({ graphData }: GraphViewProps) {
  ...
  const applyLayout = useCallback(
    (
      ...
      // Construct edge list: preserve true database edges and automatically synthesize
      // clean relationship edges from the Seed Target to any unlinked discovered nodes
      // so the graph displays clear, elegant connecting lines in all modes (Radial, Tree, Force)
      let edgeList = [...currentEdges];

      const seedNodes = layouted.filter((n) => (n.data as any)?.isSeed || n.type === 'seed');
      const rootSeed = seedNodes.length > 0 ? seedNodes[0] : null;

      if (rootSeed) {
        // Track targets that already have an incoming edge
        ...
        layouted.forEach((node) => {
          if (node.id === rootSeed.id || (node.data as any)?.isSeed || (node.data as any)?.isHub) return;
          if (!connectedTargets.has(node.id)) {
            const edgeId = `edge-seed-${rootSeed.id}-to-${node.id}`;
            ...
            edgeList.push({
              ...
              data: {
                ...
                confidence: (node.data as any)?.confidence || 85,
                reason: `Discovered from target ${rootSeed.data?.label || rootSeed.id}`,
                evidenceCount: 0,
